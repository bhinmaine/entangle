...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const convo = await sql`SELECT * FROM conversations WHERE id = ${params.id}`.then(r => r[0]);
    ...
    const sender = await sql`SELECT name FROM agents WHERE id = ${senderId}`.then(r => r[0]);
